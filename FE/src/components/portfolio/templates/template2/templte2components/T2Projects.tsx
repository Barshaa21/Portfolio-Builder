...
export default function T2Projects({ isEditRoute }: any) {
  const [isMouseOver, setIsMouseOver] = useState(false);
  const [editProductState, seteditProductStatus] = useState(false);
  const [showeditmodel, setShowEditModel] = useState(false);
  const [selectedProduct, setselectedProduct] = useState([]);
  const [addSkkils, setAddProjects] = useState(false);
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((response) => {
        ...
        console.log("showproject", response.data.result.projects);
  ...
  return (
    <div
      id="projects"
      className={`relative pb-7 min-h-[30rem]   bg-[#f0ecec] ${
    ...
    >
      ...
      <p className="text-3xl text-center underline font-bold text-[#18778F] pt-9 mr-[6rem] mb-[3rem]">
        Projects
      </p>
        ...
        {details &&
          details.map((project, index) => (
            <div key={index} className=" ">
              {isEditRoute ? (
                <div className=" mb-1 w-[2.3rem]  text-black bg-gray-300 rounded-full ">
                  {/* <i
                      onClick={() => showModel()}
                  ...
                  <i
                    ...
                    className={`fa-solid fa-trash font-extrabold text-2xl cursor-pointer p-[0.5rem] hover:text-red-800 `}
                  ></i>
                </div>
              ) : null}
              <motion.div
                className=" box border border-black shadow-white  w-[22rem] hover:bg-[#b0d8e2]  rounded-2xl h-[23rem] mb-5 bg-white"
              ...
              >
                ...
                <div className="w-[22rem] border-b-black border-l-black border-r-black border   rounded-b-2xl bg-[#b0d8e2] text-[#b0d8e2] ">
                  This div
                </div>
              </motion.div>
            </div>
